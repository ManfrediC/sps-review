...
def normalised_trimmed_text(trimmed_text: str) -> str:
    return " ".join(" ".join(trimmed_text.split()).split()).strip()
        ...
        checks.append(
            {
                ...
                "expected": display_path(case.baseline_trimmed_path),
            }
        )
    return checks
...
def evaluate_regression_case(case: RegressionCase, current_bundle: ReportBundle) -> dict[str, Any]:
    ...
    failed_end = "end_anchor_matches" in failed_names or any(
        name.startswith("not_contains::") for name in failed_names
    )
...
def run_regression_tranche(cases: list[RegressionCase], output_dir: Path) -> dict[str, Any]:
