...
# Keep the substitution rules explicit and auditable so stage 2 never smuggles
# silent free-form edits into the canonical text.
...
# Stage 2 can localize to a reviewed page span instead of reprocessing an entire
# supplement or wrapper-heavy PDF every time.
@dataclass(frozen=True)
class SourceWindow:
    start_page: int | None
    end_page: int | None
...
# The library retains its own CLI so the residual-cleanup logic can still be
# exercised directly in isolation, even though `03b_clean_text.py --stage2` is
...
# Load only enabled override rows so the reviewed rescue set remains explicit.
...
# Group substitutions by paper ID so the rescue layer only applies fixes that
# were reviewed for that exact record.
...
def build_cleanup_source_record(
    ...
    *,
    ...
    source_pdf_filename_override = str(
        override_row.get("source_pdf_filename_override") or ""
    ).strip()
    ...
    # Some residual failures are not text-layer problems at all: the attached
    # PDF for one Covidence ID is wrong, while the correct PDF already exists
    # elsewhere in the local corpus. The reviewed override lets stage 2 rebuild
    # canonical text from that alternate in-repo PDF without mutating the raw
    # downloaded files first.
    effective_source_filename = source_pdf_filename_override or source_filename
    pdf_path = PDF_DIR / effective_source_filename
